...
{
    class PacketRuleTerminate : PacketRuleBase
    {
        ...
        byte[] Terminate { get; set; } = new byte[0];

        override int WantSize => 0;
        ...
        override ReadOnlyMemory<byte> MakeSendPacket(ReadOnlyMemory<byte> bodyBuffer)
        {
        ...
        override IEnumerable<ReadOnlyMemory<byte>> MakeReceivedPacket(ReadOnlyMemory<byte> dataBuffer)
        {
        ...
        override PacketRuleBase Clone() => new PacketRuleTerminate()
        {
